...
def describe_pyproject_parser():
    ...
    def test_should_parse_setuptools(tmp_path: Path):
        ...
        assert parsers.PyProjectParser().parse(tmp_path) == BuildStep(cmd='python -m build')
    ...
    def test_should_parse_flit(tmp_path: Path):
        ...
        assert parsers.PyProjectParser().parse(tmp_path) == BuildStep(cmd='flit build')
    ...
    def test_should_parse_poetry(tmp_path: Path):
        ...
        assert parsers.PyProjectParser().parse(tmp_path) == BuildStep(cmd='poetry build')
...
def describe_package_json_parser():
    ...
    def with_package_json(tmp_path: Path):
        (tmp_path / 'package.json').write_text(json.dumps({
            'scripts': {'test': 'some test command', 'build': 'some build command'}
        }))
        yield tmp_path

    def test_should_should_parse_target_section(with_package_json: Path):
        assert parsers.PackageJsonParser('test').parse(with_package_json) == BuildStep(cmd='npm run test')

    def test_should_not_parse_if_section_not_present(with_package_json: Path):
        assert parsers.PackageJsonParser('not exist').parse(with_package_json) is None
...
def describe_docker_parser():
    ...
    def test_should_parse_with_docker(tmp_path: Path):
        ...
        assert parsers.DockerParser().parse(tmp_path) == BuildStep(cmd='docker buildx build .')
    ...
    def test_should_parse_single_docker_file(tmp_path: Path):
        ...
        assert parsers.DockerParser().parse(tmp_path) == BuildStep(cmd='docker buildx build -F Dockerfile.service .')
    ...
    def test_should_parse_dockerfile_and_other_docker_file(tmp_path: Path):
        ...
        assert parsers.DockerParser().parse(tmp_path) == BuildStep(cmd='docker buildx build -F Dockerfile .')
